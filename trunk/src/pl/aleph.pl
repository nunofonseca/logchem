...
asserta_aleph_search_node(A,B,C,D,E,F,G,H) :-
	asserta('$aleph_search_node'(A,B,C,D,E,F,G,H)).
retract_aleph_search_node(A,B,C,D,E,F,G,H) :-
	retract('$aleph_search_node'(A,B,C,D,E,F,G,H)).
retractall_aleph_search_node :-
	retractall('$aleph_search_node'(_,_,_,_,_,_,_,_)).
:- elif('$aleph_optimise_keys').
asserta_aleph_search_node(A,B,C,D,E,F,G,H) :-
	recorda(A,'$aleph_search_node'(B,C,D,E,F,G,H),_).
'$aleph_search_node'(A,B,C,D,E,F,G,H) :-
	recorded(A,'$aleph_search_node'(B,C,D,E,F,G,H),_).
retract_aleph_search_node(A,B,C,D,E,F,G,H) :-
	recorded(A,'$aleph_search_node'(A,B,C,D,E,F,G,H),R),
	erase(R).
retractall_aleph_search_node :-
	current_key(_,K),
	recorded(K,'$aleph_search_node'(_,_,_,_,_,_,_,_),R),
	erase(R),
	fail.
retractall_aleph_search_node.
:- else.
asserta_aleph_search_node(A,B,C,D,E,F,G,H) :-
	recorda('$aleph_search_node',a(A,B,C,D,E,F,G,H),_).
'$aleph_search_node'(A,B,C,D,E,F,G,H) :-
	recorded('$aleph_search_node',a(A,B,C,D,E,F,G,H),_).
retract_aleph_search_node(A,B,C,D,E,F,G,H) :-
	recorded('$aleph_search_node',a(A,B,C,D,E,F,G,H),R),
	erase(R).
retractall_aleph_search_node :-
...
expand(_,S,NodeRef,NodeRef,Path1,Length,[_],PosCover,NegCover,OVars,_,_,_):-
        retract_aleph_search_node(NodeRef,_,Path1,Length/_,_,_,OVars,_),
	get_sample_cover(S,PosCover,NegCover).
...
get_refine_gain(S,Last,Best/Node,Path,MinLength,Pos,Neg,OVars,E,Best1,NewLast):-
	...
	user_controlled_match_bot(ConstructBottom,R,R1,LitNums),
...
get_theory_gain1(S,Theory,Last,Best,Pos,Neg,P,N,Best1):-
	...
	asserta_aleph_search_node(Node1,Theory,[],0,PCvr,NCvr,[],0),
...
get_gain1(S,_,C,CL,_,Last,Best,Path,_,Pos,Neg,_,E,Best):-
	...
	arg(35,S,VSearch),
        (ClauseLength = CL -> true;
                (RefineOp = false  ->
                        asserta_aleph_search_node(Node1,0,Path,0,Pos,Neg,[],E);
			true)),
	(VSearch = true ->
...
get_gain1(S,Flag,C,CL,EMin/EL,Last,Best/Node,Path,L1,Pos,Neg,OVars,E,Best1):-
	...
	(RefineOp = false ->
		...
		(RefineOp = false ->
                	asserta_aleph_search_node(Node1,L1,Path,EMin/EL,PCvr,
					NCvr,OVars2,E);
                	asserta_aleph_search_node(Node1,0,Path,EMin/EL,PCvr,
					NCvr,[],E)),
                	update_open_list(SearchKeys,Node1,Label1)),
	(VSearch = true ->
...
rewrite_clause(Caller, Option, OriginalClause, TranslatedClause) :-
	user_clause_rewrite(OriginalClause, UserClause),
	second_rewrite_clause(Caller, Option, UserClause, TranslatedClause).
	       
second_rewrite_clause(sld,_,_,(X:-X)):- !.
second_rewrite_clause(restricted_sld,true,(Head:-Body),(Head1:-Body1)):- 
	!,
        optimise((Head:-Body),(Head1:-Body1)).
second_rewrite_clause(_,_,Clause,Clause).
...
user_controlled_match_bot(ConstructBottom,R,R1,LitNums) :-
	setting(match_bottom_hook,UserCheckRefinement),
	match_bot(ConstructBottom,R,R1,LitNums),
	call(UserCheckRefinement,R1).
...
sat_prelims:-
	...
	clean_up_hypothesis,
	reset_counts,
	setting(sat_prelims_hook,UserCheckRefine),
	( UserCheckRefine = true
	->
	  true
	;
	  call(UserCheckRefine)
	),
	set_up_builtins.
...
add_clause_to_background(ClauseIndex):-
	...
	(setting(updateback,Update) -> true; Update = true),
	user_clause_rewrite(Clause,OptimisedClause),
        (Update = true -> assertz(OptimisedClause); true),  !.
...
set_def(classes, tree,
	...
	prolog_term,'',
	show).
set_def(clause_rewrite_hook, miscellaneous,
	'Call User defined predicate before actually calling a clause, as call(P,Inp,Out)',
	prolog_term, true,
	noshow).
...
set_def(max_abducibles, search-search_space,
	...
	int(1)-int(+inf), 2,
	show).
set_def(match_bottom_hook, miscellaneous,
	'Call User defined predicate when generating a new refinement call(P,Rule)',
	prolog_term, true,
	noshow).
...
set_def(samplesize, search-search_strategy,
	...
	int(0)-int(+inf), 0,
	show).
set_def(sat_prelims_hook, miscellaneous,
	'Call User defined predicate when initialising saturation',
	prolog_term, true,
	noshow).
...
set_def(threads, miscellaneous,
	...
	int(1)-int(+inf), 1,
        show).
set_def(trace_memory_usage, miscellaneous,
	'Report on How Aleph is Using Memory',
	[true, false], false,
	show).
...
%
% call the YAP memory dumper, to have some insight on where all
% the memory goes.
trace_memory_usage :-
...
% call user rewrite 
%
user_clause_rewrite(OriginalClause, OriginalClause) :-
	setting(clause_rewrite_hook,true), !.
user_clause_rewrite(Rule, NewRule) :-
	setting(clause_rewrite_hook,Goal),
	call(Goal, Rule, NewRule), !.


lc_eval(DS,Rule):-
	read_all(DS),
	label_create(Rule,Label),
	extract_count(pos,Label,PC),
	extract_count(neg,Label,NC),
	format('~w,~w~n',[PC,NC]),
	!.
